	...
	public static MeshModel loadG3DJFromFile(String fileName)
	{
		...
		for(Object meshDesc : meshes)
		{
			...
			for(int attributeNum = 2; attributeNum < attributes.length; attributeNum++)
			{
				...
				if(attributes[attributeNum].equals("TEXCOORD0")) {  istex = true; }
				if(attributes[attributeNum].equals("TEXCOORD")) {  istex = true; }
			...
			int vertexCount;
			if(istex) {
				vertexCount = vertices.length / (8 + vertexFill);
			} else {
				vertexCount = vertices.length / (6 + vertexFill);;
			}
			...
			mesh.normals.rewind();
			if(istex) {
				mesh.uvs = BufferUtils.newFloatBuffer(vertexCount * 2);
				mesh.uvs.rewind();
			} //*/
			int bufferIndex = 0;
			for(int v = 0; v < vertexCount; v++)
			{
				...
				if(istex) {
					for(int i = 0; i < 2; i++) {
						mesh.uvs.put(vertices[bufferIndex++]);
					}
				}
